...
impl ThreatReactor {
    ...
    pub async fn react_to_threat(
        ...
        match threat_level {
            "critical" if risk_score >= 80.0 => {
                ...
                println!("🚨 CRITICAL: User {} blocked for 30 minutes", user_id);
            }
            ...
            "high" if risk_score >= 60.0 => {
                ...
                println!("⚠️ HIGH: User {} requires 2FA challenge", user_id);
            }
            ...
            "medium" if risk_score >= 40.0 => {
                ...
                println!("📊 MEDIUM: User {} rate limited (1 action / 5s)", user_id);
            }
            ...
            _ => {
                ...
                println!("✅ LOW: User {} activity logged", user_id);
            }
        }
    ...
    async fn apply_rate_limit(&self, user_id: i64, seconds: i64) {
        ...
        println!("🔒 Rate limit applied: User {} - 1 action / {}s", user_id, seconds);
    }
    ...
    pub async fn manual_unblock(&self, user_id: i64) -> bool {
        let mut blocked = self.blocked_users.lock().await;
        blocked.remove(&user_id).is_some()
    }
    ...
    pub async fn cleanup_old_entries(&self) {
        ...
        println!("🧹 Cleanup: {} rate limits, {} blocked users", limits.len(), blocked.len());
    }
}
